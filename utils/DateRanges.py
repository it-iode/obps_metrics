...
def date_ranges(date_start, date_end):
    ...
    date_list = list(OrderedDict(((start + timedelta(_)).strftime(r"%Y-%m"), None) for _ in range((end - start).days)).keys())
...
def date_ranges_month(date_start, date_end):
    ...
    dates = [date_start, date_end]
    start, end = [datetime.strptime(_, "%Y-%m-%d") for _ in dates]
    date_list = list(OrderedDict(((start + timedelta(_)).strftime(r"%Y-%m-%d"), None) for _ in range((end - start).days)).keys())
